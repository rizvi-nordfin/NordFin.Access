...
(function ($) {
    ...
    function Region(jQueryObject, globalOptions, regionID, mapsvg) {
        ...
        var regionOptions = globalOptions.regions && globalOptions.regions.ResultRegion[this.id] ? globalOptions.regions.ResultRegion[this.id] : null;
    ...
    var defaults = {
        ...
        svgFileVersion: 1,
        menu: {
            on: false, containerId: "mapsvg-menu-regions", template: function (region) {
                return '<li><a href="#' + region.id + '">' + (region.title || region.id) + '</a></li>'
            }
        },
        menuMarkers: {
            on: false, containerId: "mapsvg-menu-markers", template: function (marker) {
